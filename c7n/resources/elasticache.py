...
from c7n.filters import FilterRegistry, AgeFilter, Filter, ValueFilter
...
class VPCElasticCacheFilter(ValueFilter):
    schema = type_schema('vpc-elastic-cache-filter', rinherit=ValueFilter.schema)
    permissions = ("ecs:*",)

    def process(self, resources, event=None):
        elastic_cache_client = local_session(self.manager.session_factory).client('elasticache')
        ...
        for resource in resources:
            if 'CacheSubnetGroupName' in resource:
                vpc_id = self._is_valid_subnet_group_name(resource['CacheSubnetGroupName'])
                if vpc_id:
    ...
    def _is_valid_subnet_group_name(self, cache_subnet_group_name):
        if 'CacheSubnetGroups' in self.subnet_groups:
            for subnet_group in self.subnet_groups['CacheSubnetGroups']:
                if subnet_group['CacheSubnetGroupName'] == cache_subnet_group_name:
...
class RedisMemcacheFilter(Filter):
    ...
    def process(self, resources, event=None):
        ...
        accepted_resources = []
        self.clusters = elastic_cache_client.describe_cache_clusters(ShowCacheNodeInfo=True)
        for resource in resources:
        ...
        if 'CacheClusters' in self.clusters:
            for cluster in self.clusters['CacheClusters']:
                if 'CacheClusterId' in cluster and 'CacheClusterId' in resource and \
                    ...
                    jmespath_ports = jmespath.search('CacheNodes[].Endpoint.Port', cluster)
    ...
    def convert_value(self):
        ...
        if '-' in value:
            converted_value = [str(val) for val in range(int(value.split('-')[0]),
                                                         int(value.split('-')[1]))]
